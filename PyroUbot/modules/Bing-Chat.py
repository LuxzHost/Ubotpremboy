...
async def chat_gpt(client, message):
    try:
        ...
        if len(message.command) < 2:
            ...
            prs = await message.reply_text(f"<emoji id=5469745532693923461>♾</emoji>Proccesing King....")
            a = message.text.split(' ', 1)[1]
            response = requests.get(f'https://api.botcahx.eu.org/api/search/bing-chat?text={a}&apikey=025a6ef0')
